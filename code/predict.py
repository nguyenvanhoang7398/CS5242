from code.train import create_loaders, eval_fn, eval_ensemble_fn, load_wide_resnet, load_pretrained_densenet, \
...
from code.logistic_ensemble import logistic_ensemble, save_to_pickle, load_from_pickle
...
def predict_all(test_data):
    ...
    final_predictions = predict_fn(test_data, device, predict_transform, model_list, n_folds, ensemble_method, classifiers,
...
def predict_fn(test_data, device, predict_transform, model_list, n_folds, ensemble_method, classifiers,
               ensemble_plus_models, classifier_type, whole_classifier):
    n_models = int(len(model_list) / n_folds)
    predict_dataset, predict_loader = create_predict_loaders(predict_transform, test_data)
